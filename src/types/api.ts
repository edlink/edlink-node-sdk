...
export class BearerTokenAPI {
  ...
  async request<T>(
    ...
    if (
      ...
      this.token_set.refresh_token && this.requiresTokenRefresh()
    ) {
      // do the token refresh
      const { access_token, refresh_token } = await this.edlink.auth.refresh(
    ...
    if (data.$warnings) {
      if (this.edlink.log_level === "debug") {
        for (const warning of data.$warnings) {
          console.warn(
            `Edlink API Warning: ${warning.code} ${warning.message}`,
          );
        }
      }
    }
  ...
  public formatParams(params: RequestOptions): Record<string, any> {
    ...
    const mappings: Record<
      string,
      string | { prop: string; mod: (value: any) => any }
    > = {
      idempotency: "$idempotency",
      filter: { prop: "$filter", mod: JSON.stringify },
      properties: {
        prop: "$properties",
        mod: (value: string[]) => value.join(","),
      },
